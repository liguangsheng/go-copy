...
func memcpy(dest, src unsafe.Pointer, size uintptr) unsafe.Pointer {
	cnt := size >> 3
...
func memset(dest unsafe.Pointer, ch int8, len uintptr) unsafe.Pointer {
	left := len & 7
	cnt := len >> 3
	if cnt > 0 {
		left += 8
	...
	var i uintptr = 0
	for i = 0; i < left; i++ {
		var pdest *int8 = (*int8)(unsafe.Pointer(uintptr(dest) + uintptr(i)))
		*pdest = ch
	}
	if cnt < 2 {
		return dest
	...
	for i = 0; i < cnt-1; i++ {
		var pdest *int64 = (*int64)(unsafe.Pointer(uintptr(dest) + uintptr(left+8*i)))
		*pdest = *pfirst
	}
